...
uhero_gold      <- "#F6A01B"
uhero_green     <- "#9BBB59"
uhero_purple    <- "#8064A2" # Used for Canceled/Withdrawn projects
...
theme_uhero <- function(base_size = 14) {
  theme_minimal(base_size = base_size) +
    theme(
      ...
      axis.title = element_text(family = "Gotham", face = "bold", size = 22, color = uhero_gray),
      axis.text = element_text(family = "Gotham", size = 20, color = uhero_gray),
      legend.title = element_text(family = "Gotham", size = 22, face = "bold", color = uhero_gray),
      legend.text = element_text(family = "Gotham", size = 20, color = uhero_gray),
...
# =========================================================
# Load Permit & ROH 32 Data
permits_data <- read_csv("data/hnlpermits_with_status_0625.csv")
roh32_data <- read_csv("data/roh32_by_project_20260903.csv")
old_roh32_data <- read_csv("data/roh32_complete_cleaned (1).csv") # Used for rescuing missing unit counts
...
review_spans <- clean_plan_reviews %>%
  ...
  summarize(
    ...
    Last_Action_Date = max(end, na.rm = TRUE), 
    .groups = 'drop'
  )
...
baseline_mf_data <- permits_data %>%
  ...
  mutate(
    Project_Stage = case_when(
      status_category %in% c("Rejected/Cancelled", "Approved Then Revoked") ~ "Canceled/Withdrawn",
    ...
    Project_Stage = factor(Project_Stage, levels = c("Canceled/Withdrawn", "In Application Review", "Approved & Permitted", "Constructed (CO Issued)")),
...
# =========================================================
# PREP ROH Chapter 32 DATA (WITH UNIT RESCUE FOR CANCELED)
# =========================================================
# Create a tiny lookup table of the old units using the clean TMK
old_units_lookup <- old_roh32_data %>%
  mutate(clean_tmk = str_pad(str_remove_all(TMK, "[-:]"), width = 8, pad = "0")) %>%
  select(clean_tmk, Old_Units = Units) %>%
  distinct(clean_tmk, .keep_all = TRUE)

roh32_clean <- roh32_data %>%
  mutate(clean_tmk = str_pad(as.character(tmk), width = 8, pad = "0")) %>%
  left_join(old_units_lookup, by = "clean_tmk") %>%
  mutate(
    BP_App_Date = ymd(created_date),
    year_created = year(BP_App_Date),
    
    Project_Stage = case_when(
      status %in% c("Cancelled", "Revoked", "Canceled", "Withdrawn") ~ "Canceled/Withdrawn",
    ...
    # Rescues unit counts for canceled projects if the city zeroed them out
    Units = as.numeric(units_added),
    Units = ifelse(is.na(Units) | Units == 0, Old_Units, Units)
  ) %>%
...
baseline_plot <- ggplot(baseline_volume_summary, aes(x = year_created, y = Total_Units, fill = Project_Stage)) +
  ...
  geom_text(aes(label = ifelse(Total_Units > 0, Total_Units, "")), 
            position = position_stack(vjust = 0.5), 
            color = "white", family = "Gotham", fontface = "bold", size = 5) +
  scale_fill_manual(values = c("Constructed (CO Issued)" = uhero_green,  
                               ...
                               "Canceled/Withdrawn" = uhero_purple)) + 
  labs(
...
roh32_plot <- ggplot(roh32_volume_summary, aes(x = year_created, y = Total_Units, fill = Project_Stage)) +
  ...
  geom_text(aes(label = ifelse(Total_Units > 0, Total_Units, "")), 
            position = position_stack(vjust = 0.5), 
            color = "white", family = "Gotham", fontface = "bold", size = 6) +
  ...
  annotate("text", x = 2019, y = 520, label = "Policy Goal (500 units constructed/year)", 
           color = uhero_green, family = "Gotham", fontface = "bold", hjust = 0, size = 7) +
...
baseline_time_summary <- baseline_mf_data %>%
  filter(!is.na(processing_days), was_approved == TRUE, Project_Stage != "Canceled/Withdrawn") %>%
...
roh32_time_summary <- roh32_with_time %>%
  filter(!is.na(processing_days), Project_Stage != "Canceled/Withdrawn") %>% 
  group_by(year_created, Group, Data_Status) %>%
  summarize(Median_Days = median(processing_days, na.rm = TRUE), .groups = "drop")
...
active_clean_times <- roh32_clean %>%
  # Exclude zombies and canceled projects from the time math
  filter(!is.na(DPP_Days), Is_Zombie == FALSE, Project_Stage != "Canceled/Withdrawn") %>%
  group_by(year_created) %>%
  summarize(
...
dpp_isolation_plot <- ggplot(active_clean_times, aes(x = year_created, y = Average_Days, fill = Time_Type)) +
  geom_bar(stat = "identity", position = "stack", width = 0.65) +
  geom_text(aes(label = ifelse(Average_Days > 0, round(Average_Days), "")), 
            position = position_stack(vjust = 0.5), 
            color = "white", family = "Gotham", fontface = "bold", size = 5) +
